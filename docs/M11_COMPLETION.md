...
| Метаданные App Server / Desktop проверены во всех фазах | **Частично.** Размещение задачи в проекте наблюдалось живьём во всех шести воркерах приёмки; `desktop_placement` переписан на `thread/read` и сверку `projectId`. |
...
| 2 | `M11-PRE-SIDE-EFFECT-FENCE` | Открыт. Отказ отставленной задаче на `UserPromptSubmit` до любого побочного эффекта не реализован. |
...
| 4 | `M11-R5-DESKTOP-PLACEMENT` | Частично. Точная сверка принадлежности проекту сделана; нормализация устойчивого расхождения в один инцидент вместо вечного `IN_PROGRESS` - нет. |
| 5 | `M11-R6-FAIL-CLOSED` | Открыт. `ensure_project_root` при каждом создании молча вызывает `project/update`, добавляя канонический корень в сохранённый проект. Должен отказывать, пока мутацию не разрешит записанное решение пользователя. |
| 6 | `M11-R7-ATTRIBUTION-BUDGET` | Открыт. Учёта общего дерева по задачам и бюджетов времени/попыток/токенов нет. |
| 7 | `M11-R13-UNIFIED-REASONS` | Частично. У эскалации инженера код из закрытого списка обязателен. У воркера `BLOCKED` кода нет: пишется свободная строка `last_error`. |
| 8 | `M11-R16-R17-PHASE-CONTRACT` | Открыт для проверяющего, реплэннера и инженера. |
| 9 | `M11-R18-TAINT` | Частично. Путь решения закрыт: принятое не-пользовательское решение, опирающееся на внешний текст, отвергается. Обязательность происхождения при приёме и переоценка на переходах Constraint/Truth и на вводе MCP - нет. |
...
Три пункта - 5, 7 и частично 4 - сознательно не тронуты сейчас: они
лежат в пути создания и завершения задачи, то есть ровно там, где
работает готовящийся чистый прогон. Менять этот путь за час до прогона,
который должен его проверить, значит проверять уже не то, что дважды
прошло приёмку. Они делаются после прогона.
