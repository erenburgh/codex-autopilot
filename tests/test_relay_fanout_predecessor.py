"""B8: у «ход владельца закончен» один предикат, а не два.

Аудит нашёл три реализации «ход закончен», две из них расходились.
Замерено 18.09 разбором всех пяти кандидатов: три из них - другие
вопросы (гейт запуска, форензика ретрая, аудит R1), и различия там
намеренные. Настоящий дрейф - в одном месте: ветка веера
``cli._automatic_relay_loop`` (несколько преемников) выбирала
предшественника inline по ``status == "COMPLETED"``, а одиночная ветка
той же функции и Stop-хук идут через ``causal_predecessor`` -
``_turn_is_completed``, который принимает и завершённый ход с
BLOCKED/ESCALATE/PLAN_CHANGE_REQUESTED, и прерванный.

Сценарий достижим: ``complete_desktop_worker`` при BLOCKED оставляет
``status = worker_status`` (lifecycle_completion.py) и в той же
транзакции пишет ``turn_completed``. Импликация COMPLETED => turn_completed
верна, обратная - нет по построению. Если такой владелец открыл двух
преемников, веер падал голым ``StopIteration`` - без слова о причине.
"""
