...
class PlanChangePredecessorIsAcceptedTests(unittest.TestCase):
    ...
    # «Барьер диспетчера пользуется общим предикатом» проверяется теперь
    # исполнением в test_desktop_lifecycle: подменённый causal_predecessor
    # роняет adopt_automatic_dispatcher_successor тем самым отказом.
