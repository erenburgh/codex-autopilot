    ...
    def _render_prompt(
        ...
        *,
        ...
        if phase == "verification":
            ...
            finish = (
                f"Запиши новое проверяемое evidence для {task.id}; для evidence checks используй точный check ID как role. Заверши ровно одной строкой AUTOPILOT_STATUS: ROTATE, BLOCKED или ESCALATE. Если обнаружена необходимая смена prerequisite/dependency/resource/verification контракта, вместо status верни ровно одну финальную строку PLAN_CHANGE_REQUEST с JSON-полями request_version=1, kind, target_task_id={task.id}, summary, rationale, change и evidence_ids."
                if russian
                else f"Record new verifiable evidence for {task.id}; use each exact check ID as the role for evidence checks. Finish with exactly one AUTOPILOT_STATUS: ROTATE, BLOCKED, or ESCALATE line. If a prerequisite/dependency/resource/verification contract change is required, return exactly one final PLAN_CHANGE_REQUEST line instead, with JSON fields request_version=1, kind, target_task_id={task.id}, summary, rationale, change, and evidence_ids."
            )
