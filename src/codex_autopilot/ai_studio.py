...
- `scripts/codex-autopilot devops-repair-runtime --project <root> --incident-id <id> --module <file.py> --old-file <path> --new-file <path> --test-file <path> --test-name test_<name>` — repair the runtime's own code when the task is blocked by a defect in Autopilot itself, not in the project. You do not declare the repair: the gateway proves it. Write the exact fragment to replace into `--old-file` (it must occur exactly once in that module), its replacement into `--new-file`, and a reproduction test into `--test-file`. The gateway copies the runtime aside and requires all of: the reproduction test FAILS on the current code, PASSES with the patch, the whole suite stays green, and the guarded ownership and trust definitions stay byte-identical. Anything else and the installation is untouched. An accepted patch takes effect on the next dispatched turn — nothing is reinstalled under a running dispatcher. Report it as `--action repair_runtime_code`. Never use this on the project's own code: fixing the product is the workers' job, not yours.
