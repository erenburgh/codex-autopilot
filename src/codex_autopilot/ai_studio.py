    ...
    def _render_prompt(
        ...
        *,
        ...
        if phase == "verification":
            ...
            finish = (
                f"Запиши новое проверяемое evidence для {task.id}; для evidence checks используй точный check ID как role. Перед финальной строкой дай строку AUTOPILOT_RULES с id правил из блока rules, которые ты применила к этой задаче (например AUTOPILOT_RULES: R7, R17). Заверши ровно одной строкой AUTOPILOT_STATUS: ROTATE, BLOCKED или ESCALATE; DONE допустим только для последней задачи плана. Если обнаружена необходимая смена prerequisite/dependency/resource/verification контракта, вместо status верни ровно одну финальную строку PLAN_CHANGE_REQUEST с JSON-полями request_version=1, kind, target_task_id={task.id}, summary, rationale, change и evidence_ids."
                if russian
