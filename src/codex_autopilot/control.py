...
def _launch_report(
    ...
    *,
    ...
    if verdict is not LaunchVerdict.FAILED:
        ...
        #
        # Но показывать чек-лист надо всегда: systemMessage при continue
        # пользователю не виден, и успешный запуск выглядел такой же
        # тишиной, как молчаливый затык. Поэтому ответ блокирующий - он
        # доносит отчёт, - а запуску это не мешает: диспетчер уже поднят
        # отдельным процессом и от завершения этого хода не зависит.
