...
- signalled once per cause per run as a ticket that holds no task
  ...
  is hers (a Desktop project root, the isolation trade-off) goes up with its
  diagnosis, a recommendation, and the run's threads that are outside the
  project by id and title - she can move them in Desktop herself. A ticket
  per thread would raise an on-call per launch for one cause.
"""
...
def _diagnosis(cause: str, observation: Mapping[str, Any]) -> tuple[str, str, str]:
    """Diagnosis, recommendation and the code a decision of hers goes up with."""
    ...
    if cause == "isolation_not_proven":
        return (
            "Staged tasks keep their staged workspace as cwd because the isolation probe did "
            "not prove the canonical root stays read-only with cwd = root "
            "(.codex-autopilot/isolation-probe.json); Desktop files them in no project.",
            "Read isolation-probe.json. A probe that could not run is repaired and re-measured "
            "(delete the record; the dispatcher measures again). If the root is writable, the "
            "choice is hers: keep isolated tasks outside the project, or give the run a "
            "permission profile that denies writes to the root.",
...
def _file_once(cfg: Any, state: Any, *, cause: str, task_id: str, kind: str, after: str,
               observation: Mapping[str, Any], at: str, defect: Mapping[str, Any]) -> str | None:
    """One ticket per cause per run; the run's outside threads listed in it."""
    ...
    filed = [
        ...
        and (item.get("system_state") or {}).get("cause") == cause
        and str((item.get("system_state") or {}).get("run_id") or "") == str(state.run_id or "")
    ]
    if filed:
        return None
    diagnosis, recommendation, code = _diagnosis(cause, observation)
    return stop_run(
        cfg,
        state,
        stop_kind=STOP_KIND,
        ...
        summary=f"R5: a {kind} thread of {task_id} is not in the project in Desktop. Nothing is held: the run goes on.",
        at=at,
        task_ids=(),
        ...
        system_state={
            ...
            "defect": dict(defect),
            "outside_threads": outside_threads(cfg, state, observation.get("codex_home")),
        },
    )
