...
PLACEMENT_TICKET_BRIEF = """This stop is an R5 placement or isolation defect (`stop_context.stop_kind` is placement_defect): a thread of the run is not filed in the project in Desktop, or a thread filed at the root was widened or left the canonical root changed. It holds no task - the run goes on. `stop_context.placement` has the cause, the separate facts (App Server projectId ok, Desktop's own rule and reason, the Desktop version it was measured on), a diagnosis, a recommendation and `outside_threads` - every thread of the run Desktop files outside the project, by id and title; `unobserved_threads` are those whose placement could not be read (no Desktop state): never report them as outside. A thread filed by the runtime with the wrong cwd is a runtime defect: repair it with devops-repair-runtime and close naming repair_runtime_code. Isolation is the runtime's, never a choice for her: cause isolation_not_proven means the staged permission profile was not proven on this Codex binary (.codex-autopilot/isolation-probe.json) - the dispatcher measures again by itself (a NOT_PROVEN after its retry interval, any record once the runtime code or the binary differs), so a NOT_PROVEN that keeps coming back or a ROOT_WRITABLE is yours to repair with devops-repair-runtime: once installed, the next staged thread is measured on the repaired code; runtime_roots_widened and canonical_changed_outside_manifest name the thread, the roots and the paths - a write by a thread is an isolation breach to repair. Never write Desktop's state file and never answer an approval. A missing Desktop project root (R6) is hers: escalate with `stop_context.reason_code`, the diagnosis, your recommendation and the listed threads (she can move them into the project in Desktop); so is a canonical change you cannot explain or repair (PROJECT_DAMAGE_RISK)."""
...
def read_engineer_outcome(cfg: Any, incident_id: str, final_message: str) -> tuple[str, str, str]:
    ...
    if (
        ...
        and code == "DANGEROUS_PERMISSION"
        and (incident.get("system_state") or {}).get("approval_class") == "root_write"
    ):
        # A write into the root the runtime filed the thread at: its
        # placement's defect, classified from the request (approval_stops),
        # not a permission she could sensibly grant.
        return (
            "PROTOCOL_ERROR",
            "",
            "the request is a write under the project root from a thread the runtime filed there "
            "(approval_class root_write): a runtime defect of placement, never her permission. "
            "Repair it (devops-repair-runtime), or escalate RECOVERY_EXHAUSTED with your diagnosis",
        )
