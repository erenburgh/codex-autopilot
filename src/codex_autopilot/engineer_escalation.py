...
PLACEMENT_TICKET_BRIEF = """This stop is an R5 placement defect (`stop_context.stop_kind` is placement_defect): a thread of the run is not filed in the project in Desktop. It holds no task - the run goes on. `stop_context.placement` has the cause, the separate facts (App Server projectId ok, Desktop's own rule and reason, the Desktop version it was measured on), a diagnosis, a recommendation and `outside_threads` - every thread of the run Desktop files outside the project, by id and title. A thread filed by the runtime with the wrong cwd is a runtime defect: repair it with devops-repair-runtime and close naming repair_runtime_code. Never write Desktop's state file and never answer an approval. A missing Desktop project root, or the choice between isolated staged tasks outside the project and a permission profile that denies writes to the root, is hers: escalate with `stop_context.reason_code`, the diagnosis, your recommendation and the listed threads (she can move them into the project in Desktop)."""
