...
def initialize_project(
    ...
    *,
    ...
    # R28: перезапись состояния без снимка отклоняется. Снимок делается
    # ДО первой мутации - ниже сносятся логи и перезаписываются план,
    # состояние, конфиг и ROADMAP.md. Копия, а не перенос: память и
    # handoff/ обязаны остаться жить у нового прогона. Без прежнего
    # run-state снимать нечего: копия пустого прошлого - мусорный сосед
    # на каждом первом запуске.
    previous_state = (
    ...
    if previous_state is not None:
        # Журнал прежнего состояния уехал вместе с ним; путь записывается
        # в первое же событие нового - иначе снимок восстановим только по
        # памяти о том, что он был.
...
def purge_project_state(root: Path) -> Path | None:
    ...
    if not state_dir.exists():
        return None
    # Живой прогон не сносится: uninstall останавливает диспетчер сам, но
    # функция публичная, и страж стоит там, где данные.
    state_path = state_dir / "run-state.json"
    try:
...
def archive_state_dir(state_dir: Path, *, reason: str, move: bool) -> Path:
    ...
    if move:
        ...
        # ROADMAP.md лежит в корне проекта и перезаписывается вместе с
        # состоянием - без него снимок неполон.
